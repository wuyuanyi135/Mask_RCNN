"""
Mask R-CNN
Train on the toy crystal dataset.

Copyright (c) 2018 Matterport, Inc.
Licensed under the MIT License (see LICENSE for details)
Written by Yuanyi Wu
...
class CrystalDataset(utils.Dataset):

    def load_crystal(self, dataset_dir, subset):
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = CrystalDataset()
    dataset_train.load_crystal(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = CrystalDataset()
    dataset_val.load_crystal(args.dataset, "val")
    dataset_val.prepare()
